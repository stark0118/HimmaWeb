...
//初始化区域图的对象
	...
	var width=(property.width||1200)-2;
	var height=(property.height||800)-2;
	...
	if(this.$editable){
	  ...
	  if(property.useOperStack){//如果要使用堆栈记录操作并提供“撤销/重做”的功能
		...
		this.start = function () {
			window.confirm("是否启动调度");
		};
		this.open=function () {
			jsondata={
				title:"推荐方案",
				nodes:{
					demo_node_1:{name:"ip:192.168.61.133",left:67,top:69,type:"start",width:24,height:24},
					demo_node_2:{name:"redis0",left:219,top:71,type:"task",width:86,height:24},
					demo_node_5:{name:"redis1",left:380,top:71,type:"task",width:86,height:24}
				},
				lines:{
				...
				areas:{
					demo_area_1: {name:"方案_1",left:35,top:39,color:"red",width:472,height:200}
				}
			};
			demo.$max=9;
